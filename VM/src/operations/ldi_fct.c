...
int			ldi_fct(t_war *war, t_chariot *chariot)
{
	...
	if (war->rtype[0] == T_REG)
		param1 = chariot->registres[war->arena[chariot->start_pos + chariot->pc + i] - 1];
	if (war->rtype[0] == T_DIR)
		param1 = get_2_val(war, chariot, i);
	if (war->rtype[1] == T_REG)
	{
		if (war->rtype[0] == T_REG)
			param2 = chariot->registres[war->arena[chariot->start_pos + chariot->pc + i + 1] - 1];
		else
			param2 = chariot->registres[war->arena[chariot->start_pos + chariot->pc + i + 2] - 1];
	}
	if (war->rtype[1] == T_DIR)
		param2 = get_2_val(war, chariot, i);
	p3 = (param1 + param2) % IDX_MOD;
	p3 = (chariot->start_pos - i + p3) % MEM_SIZE;
	chariot->registres[war->arena[(chariot->start_pos + chariot->pc + i) % MEM_SIZE]] = p3;
	//je crois que chariot->registres[x] s'atteint avec x = pos du registres actuel ?!
	return (0);
}
