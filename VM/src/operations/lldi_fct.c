...
int			lldi_fct(t_war *war, t_chariot *chariot)
{
    ...
    if (war->rtype[0] == IND_CODE)
    {
        ...
        if (war->rtype[1] == REG_CODE)
        {
            ...
            if (war->verbose[2] == 1 && r > 0 && r <= 16)
            {
                ...
                printf("       | -> load to  %hd + %d = %d (with pc and mod %d)\n", param1, war->tmp, param1 + war->tmp, calc_addr(chariot->pc + ((param1 + war->tmp) % IDX_MOD)));
            }
    ...
    if (war->rtype[0] == REG_CODE)
    {
    	...
    	if (war->rtype[1] == REG_CODE)
    	{
            ...
            if (war->verbose[2] == 1 && r > 0 && r <= 16)
            {
                ...
                printf("       | -> load to %d + %d = %d (with pc and mod %d)\n", param1, param2, param1 + param2, calc_addr(chariot->pc + ((param1 + param2) % IDX_MOD)));
            }
            if (r > 16 || r <= 0)
            {
    ...
    p3 = calc_addr((chariot->pc - i + p3) % MEM_SIZE);
    if (p3 == 0)
    	chariot->carry = 1;
    else
    	chariot->carry = 0;
